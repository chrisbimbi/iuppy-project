...
class ApiClient {
  ...
  Future<List<Map<String, dynamic>>> getCompanyModules() async {
    ...
    return List<Map<String, dynamic>>.from((resp.data as List).map((e) => Map<String, dynamic>.from(e as Map)));
  }

  Future<List<Map<String, dynamic>>> getSpaces() async {
    final resp = await _dio.get('/spaces', queryParameters: {'companyId': companyId});
    return List<Map<String, dynamic>>.from((resp.data as List).map((e) => Map<String, dynamic>.from(e as Map)));
  }

  Future<List<Map<String, dynamic>>> getChannels({String? spaceId}) async {
    final qp = {'companyId': companyId, if (spaceId != null) 'spaceId': spaceId};
  ...
  Future<List<Map<String, dynamic>>> getNewsByChannel(String channelId) async {
    // CMS references use '/news' with ?channelId=. Using that shape here.
    final resp = await _dio.get('/news', queryParameters: {'channelId': channelId});
    return List<Map<String, dynamic>>.from((resp.data as List).map((e) => Map<String, dynamic>.from(e as Map)));
  }
  ...
  Future<void> postSurveyResponse(String id, Map<String, dynamic> body) async {
    await _dio.post('/modules/$companyId/surveys/$id/responses', data: body);
  }
}

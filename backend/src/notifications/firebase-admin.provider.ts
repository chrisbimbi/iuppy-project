...
import { initializeApp, applicationDefault, cert, App, getApps } from 'firebase-admin/app'
...
export const FirebaseAdminProvider: Provider[] = [
  {
    ...
    useFactory: () => {
      ...
      log.log(`Iniciando Firebase Admin via ${usingADC ? 'Application Default Credentials (arquivo JSON)' : 'ENV PRIVATE_KEY'}`)

      const app: App =
        getApps()[0] ??
        initializeApp(
          usingADC
            ? { credential: applicationDefault(), projectId: projectId || undefined }
            : {
                credential: cert({
                  projectId: projectId!,

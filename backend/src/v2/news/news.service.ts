...
type ModerationMode = 'approved' | 'status' | 'none'
...
function sanitizeAvatar(url: string | null): string | null {
  ...
  if (isHttp) {
    try {
      ...
      // bloquear hosts que dão 403 sem cookie/assinatura
...
export class NewsV2Service {
  ...
  // ------------------------------------------------------------------
  // Detecção robusta de moderação em news_comment
  // ------------------------------------------------------------------
  private async detectModerationMode(): Promise<ModerationMode> {
    if (await this.hasColumn('news_comment', 'approved')) return 'approved'
    if (await this.hasColumn('news_comment', 'status')) return 'status'
    return 'none'
  }

  /**
   * Retorna o trecho de WHERE para moderação conforme modo e status desejado.
   * - Para feed/app queremos apenas aprovados quando houver moderação.
   * - Para admin, aplicamos o status selecionado.
   */
  private buildModerationWhere(mode: ModerationMode, opts: { admin?: boolean; status?: CommentStatus }): { where: string[] } {
    const where: string[] = []
    const { admin = false, status } = opts

    if (!admin) {
      // modo app: só aprovados quando houver moderação
      if (mode === 'approved') where.push(`c."approved" = TRUE`)
      else if (mode === 'status') where.push(`UPPER(c."status"::text) IN ('APPROVED')`)
    } else if (status && status !== 'all') {
      // modo admin: respeita filtro
      if (mode === 'approved') {
        if (status === 'pending') where.push(`c."approved" IS NULL`)
        if (status === 'approved') where.push(`c."approved" = TRUE`)
        if (status === 'rejected') where.push(`c."approved" = FALSE`)
      } else if (mode === 'status') {
        if (status === 'pending') where.push(`(c."status" IS NULL OR UPPER(c."status"::text) IN ('PENDING'))`)
        if (status === 'approved') where.push(`UPPER(c."status"::text) IN ('APPROVED')`)
        if (status === 'rejected') where.push(`UPPER(c."status"::text) IN ('REJECTED')`)
      }
      // mode none => não filtra moderação
    }
  ...
  /**
   * Detecção do metadado da tabela de eventos OPEN/ACK
   */
  private async detectEventMeta(): Promise<EventMeta> {
    ...
    // Fallback “hardcoded” — o que seus logs mostram existir
  ...
  private async fetchPreviewComments(companyId: string, newsId: string, limit = 6) {
    const mode = await this.detectModerationMode()
    const where = [`c."companyId"=$1`, `c."newsId"=$2`]
  ...
  async comment(companyId: string, newsId: string, userId: string, text: string) {
    ...
    const moderateComments =
      (settings?.commentsRequireModeration ??
        settings?.moderation?.comments ??
  ...
  // ---------- comments list (app/public com cursor)

  async listCommentsPublic(
    companyId: string,
    newsId: string,
    ...
    const where = [`c."companyId"=$1`, `c."newsId"=$2`]
    const mode = await this.detectModerationMode()
    const { where: mod } = this.buildModerationWhere(mode, { admin: false })
    where.push(...mod)

    if (cursor) {
      where.push(`c."createdAt" < $3`)
  ...
  // ---------- comments list (admin/painel com paginação por página e filtros)
  ...
  async listCommentsAdmin(
    ...
    const { status, q, from, to, page, pageSize } = opts
    const params: any[] = [companyId, newsId]
    let idx = 3
    const where: string[] = [`c."companyId"=$1`, `c."newsId"=$2`]

    // período
    if (from) {
      where.push(`c."createdAt" >= $${idx++}::timestamp`)
      params.push(new Date(from))
    }
    if (to) {
      where.push(`c."createdAt" < ($${idx++}::date + INTERVAL '1 day')`)
      params.push(new Date(to))
    }

    // moderação (admin)
    const mode = await this.detectModerationMode()
    const { where: mod } = this.buildModerationWhere(mode, { admin: true, status })
    where.push(...mod)

    // busca (texto / nome / email)
    let qSql = ''
    const qNorm = (q ?? '').trim()
    if (qNorm) {
      qSql = ` AND (
        c."text" ILIKE $${idx}
        OR COALESCE(NULLIF(u."displayName", ''), NULLIF(u."name", ''), u."email") ILIKE $${idx}
      )`
      params.push(`%${qNorm}%`)
      idx += 1
    }
    ...
    const countSql = `
      ...
      FROM news_comment c
      JOIN user_entity u ON u.id::text = c."userId"::text
      WHERE ${baseWhere} ${qSql}
    `
    const totalRows = await this.newsRepo.query(countSql, params)
    const total = totalRows?.[0]?.c ?? 0

    // projeção de "approved" quando o modo é status
    const selectApproved =
      mode === 'approved'
        ? `, c."approved" AS "approved"`
        : mode === 'status'
          ? `, CASE WHEN c."status" IS NULL THEN NULL ELSE (UPPER(c."status"::text)='APPROVED') END AS "approved"`
    ...
    `
    const rows = await this.newsRepo.query(listSql, params)
    ...
    const items = (rows ?? []).map((r: any) => ({
      ...
      avatar: sanitizeAvatar(r.avatar ?? null),
      approved:
        r.approved === null || typeof r.approved === 'undefined'
          ? null
          : Boolean(r.approved),
    }))
  ...
  // ---------- compat: assinatura antiga usada no controller público
  async listComments(
    companyId: string,
    newsId: string,
    opts: { limit: number; cursor?: string },
  ) {
    return this.listCommentsPublic(companyId, newsId, opts)
  }

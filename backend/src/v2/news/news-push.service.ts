...
export class NewsPushServiceV2 {
  ...
  private async resolveAudience(companyId: string, newsId: string, settings: any): Promise<string[]> {
    ...
    if (fromNA !== null) {
      if (fromNA.length > 0) {
        this.logger.log(`[audience] news_audience (explicit) → ${fromNA.length} users`)
        return fromNA
      }
      if (visibility === 'public' || visibility === 'all_users') {
        const all = await this.selectAllCompanyUsers(companyId)
        this.logger.log(`[audience] news_audience vazio + visibility=${visibility} → all users: ${all.length}`)
        return all
      }
      this.logger.warn(`[audience] news_audience vazio + visibility=${visibility} (segmentado) → audiência vazia`)
      return []
    }

    if (visibility === 'public' || visibility === 'all_users') {
      const all = await this.selectAllCompanyUsers(companyId)
      this.logger.log(`[audience] sem news_audience + visibility=${visibility} → all users: ${all.length}`)
      return all
    }

    this.logger.warn(`[audience] sem news_audience + visibility=${visibility} (segmentado) → audiência vazia`)
    return []
  }
  ...
  private async filterOnlyNotOpened(
    ...
    if (!ev) {
      this.logger.warn('[filter] v2 events map não encontrado — sem filtro OPEN/ACK')
      return userIds
    }
  ...
  async send(
    ...
    if (opts.testToken && typeof opts.testToken === 'string') directTokens.push(opts.testToken)
    ...
    if (directTokens.length > 0) {
      this.logger.warn(`[${reqId}] [send][TEST] tokens=${directTokens.length} — ignorando audiência`)
    ...
    if (!notify && !opts.testUserId) {
      this.logger.warn(`[${reqId}] send: pushNotification OFF → skip (newsId=${newsId})`)

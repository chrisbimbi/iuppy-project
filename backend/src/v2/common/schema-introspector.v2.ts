  ...
  async getLastUpdateMarker(companyId: string, fromISO?: string, toISO?: string): Promise<string> {
    ...
    const addMax = async (
      ...
      const addRange = (baseIndex: number) => {
        let idx = baseIndex
        if (fromISO) {
          sql += params.length ? ` AND ${c} >= $${idx}` : ` WHERE ${c} >= $${idx}`
          params.push(fromISO)
          idx++
        }
        if (toISO) {
          sql += params.length ? ` AND ${c} < ($${idx}::date + INTERVAL '1 day')`
    ...
    if (await this.tableExistsRaw('news_reaction')) {
      const col = (await this.columnExistsRaw('news_reaction', 'createdAt')) ? 'createdAt' : 'created_at'
    ...
    if (await this.tableExistsRaw('news_entity') && await this.columnExistsRaw('news_entity', 'updatedAt')) {
      await addMax('news_entity', 'updatedAt', { withCompanyId: true })
    }

    // 6) Métricas diárias (pode não ter companyId; usamos apenas o range)
    if (await this.tableExistsRaw('news_metrics_daily') && await this.columnExistsRaw('news_metrics_daily', 'date')) {

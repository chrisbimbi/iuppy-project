...
// ADICIONADO 'pt-BR' AQUI
const LOCALES = ['pt', 'pt-BR', 'en', 'es', 'de'] as const;
...
class BrandingDto {
  ...
  // cores hex #RRGGBB
  @IsOptional()
  @Matches(/^#([0-9A-Fa-f]{6})$/, { message: 'primary deve ser hex (#RRGGBB)' })
  primary?: string;
...
export class UpsertCompanySettingsDto {
  ...
  @IsIn(LOCALES as unknown as string[], {
    message: `defaultLocale deve ser um de: ${LOCALES.join(', ')}`,
  })
  ...
  @IsIn(LOCALES as unknown as string[], {
    each: true,
    message: `Cada supportedLocale deve ser um de: ${LOCALES.join(', ')}`,
  })

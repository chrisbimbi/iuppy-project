...
export class CompanySettingsService {
  constructor(
    ...
    private repo: Repository<CompanySettingsEntity>,
  ) {}
  ...
  async upsert(
    companyId: string,
    dto: UpsertCompanySettingsDto,
  ): Promise<CompanySettings> {
    const curr =
      (await this.repo.findOne({ where: { companyId } })) ||
      this.repo.create({ companyId });
    ...
    if (dto.menuConfig) {
      curr.menuConfig = dto.menuConfig;
    }
  ...
  private map(e: CompanySettingsEntity): CompanySettings {
    return {
      ...
      // ATUALIZADO AQUI: incluído pt-BR na lista de fallback
      supportedLocales: (e.supportedLocales?.length
        ? e.supportedLocales
        : ['pt-BR', 'pt', 'en', 'es', 'de']) as any,
      branding: {
        logoUrl: e.logoUrl,
        appTitle: e.appTitle,

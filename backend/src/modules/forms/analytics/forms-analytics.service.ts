...
export class FormsAnalyticsService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log(`[FormsAnalyticsService] Tables: ${JSON.stringify(this.tableNames)}`);
  }

  // @ts-ignore
  public normalizeRange(q: { from?: string; to?: string }): { from: string; to: string } {
    const now = new Date();
    const to = q.to ? new Date(q.to) : new Date();
    to.setHours(23, 59, 59, 999);
    const from = q.from ? new Date(q.from) : new Date(new Date().setDate(now.getDate() - 7));
  ...
  private buildFilterWhere(q: DateRange, alias: string, paramIndexStart: number) {
    const params: any[] = [];
    const wheres: string[] = [];
    ...
    return { where: wheres.length > 0 ? `AND ${wheres.join(' AND ')}` : '', params };
  }
  ...
  async runDailyAggregation(companyId: string, dateStr: string, formId?: string) {
    ...
    const startOfDay = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 0, 0, 0, 0)).toISOString();
    const endOfDay = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 23, 59, 59, 999)).toISOString();

    const formFilter = formId ? `AND f.id = $2::uuid` : '';
    const formParams = formId ? [companyId, formId] : [companyId];
    const forms = await this.safeQuery(`SELECT id FROM form f WHERE f."companyId" = $1 ${formFilter}`, formParams);
    let totalForms = 0;
    ...
    for (const form of forms) {
      ...
      await this.safeQuery(
        ...
        [companyId, form.id, dateStr, m.impressions || 0, m.opens || 0, m.starts || 0, m.submits || 0, m.onTimeSubmits || 0, m.internalSubmits || 0, m.externalSubmits || 0, m.pushSent || 0, m.pushOpened || 0, m.emailSent || 0, m.emailOpened || 0, m.emailClicked || 0]
      );
      totalForms++;
    }
  ...
  async runManualAggregation(companyId: string, date: string, formId?: string) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new BadRequestException('Invalid date format');
  ...
  async overview(companyId: string, q: DateRange) {
    ...
    const getResult = (i: number, def: any = []) => (results[i].status === 'fulfilled' ? (results[i] as any).value : def);
  ...
  async list(companyId: string, q: any) {
    ...
    const totalRow = await this.safeQuery(`SELECT COUNT(*) AS c FROM form f WHERE f."companyId"=$1 ${statusFilter}`, params);
  ...
  async formStats(companyId: string, formId: string, q: DateRange) {
    ...
    return {
      ...
      kpis: {
        ...
        firstResponseMsP50: getResult(12, true)?.[0]?.firstResponseMsP50 ?? null,
      },
      series: {
      ...
      segments: {
        ...
        byAudience: [{ type: 'internal', submits: Number(k.submissions || 0) - Number(k.external || 0) }, { type: 'external', submits: Number(k.external || 0) }],
      },
  ...
  async getFieldsStats(companyId: string, formId: string, q: DateRange): Promise<FieldStat[]> {
    const { from, to } = this.normalizeRange(q);
    const fields = await this.safeQuery(`SELECT id, label, type, options FROM "form_field" WHERE "companyId"=$1 AND "formId"=$2::uuid ORDER BY "order" ASC`, [companyId, formId]);
    ...
    for (const f of fields) {
      ...
      const fMetrics = await this.safeQueryOptional(`SELECT COUNT(*) FILTER (WHERE e.type='field_focus') AS focus, COUNT(*) FILTER (WHERE e.type='field_change') AS changes, COUNT(*) FILTER (WHERE e.type='field_validation_error') AS "validationErrors" FROM form_event e WHERE e."companyId"=$1 AND e."formId"=$2::uuid AND e."fieldId"=$3::uuid AND e.ts>=$4::timestamptz AND e.ts<($5::date + INTERVAL '1 day')`, metricsParams);
      const m = fMetrics[0] ?? { focus: 0, changes: 0, validationErrors: 0 };

      let distribution: any = undefined;
      let wordCloud: any = {};

      if (['single_choice', 'multi_choice', 'stars', 'scale'].includes(f.type)) {
        ...
        if (f.type === 'multi_choice') {
          ...
          distData.forEach((row: any) => {
            if (Array.isArray(row.value)) {
              row.value.forEach((v: string) => counts.set(v, (counts.get(v) ?? 0) + row.count));
            } else {
              const v = String(row.value);
              counts.set(v, (counts.get(v) ?? 0) + row.count);
            }
          });
          distribution = { choices: Array.from(counts.entries()).map(([value, count]) => ({ value, count })) };
        } else {
          distribution = { choices: distData };
        }
      }
      if (['short_text', 'long_text'].includes(f.type)) {
        const textData = await this.safeQueryOptional(`SELECT a.value FROM form_answer a JOIN form_submission s ON s.id=a."submissionId" WHERE a."companyId"=$1 AND s."formId"=$2::uuid AND a."fieldId"=$3::uuid ${filters.where}`, [...baseParams, ...filters.params]);
        wordCloud = this.processWordCloud(textData);
      }

      results.push({
        fieldId: f.id, label: f.label, type: f.type,
        metrics: {
          focus: Number(m.focus), changes: Number(m.changes), validationErrors: Number(m.validationErrors),
          errorRate: Number(m.focus) > 0 ? Number(m.validationErrors) / Number(m.focus) : 0,
          avgTimeMs: 0, abandonAfterErrorRate: 0,
        },
        distribution, ...wordCloud
      });
    }
    return results;
  }

  private processWordCloud(textData: Array<{ value: any }>): { topWords: any[], bigrams: any[], trigrams: any[] } {
    const allText = textData.map((r) => r.value).filter((v) => typeof v === 'string' && v.trim().length > 2).join(' ');
    if (allText.length === 0) return { topWords: [], bigrams: [], trigrams: [] };
    const tokenizer = new natural.WordTokenizer();
    const tokens = tokenizer.tokenize(allText.toLowerCase());
    const stopwords = new Set(natural.stopwords);
    const validTokens = tokens.filter((t) => t.length > 2 && !stopwords.has(t) && isNaN(Number(t)));
    const tfidf = new natural.TfIdf();
    tfidf.addDocument(validTokens);
    const topWords = tfidf.listTerms(0).slice(0, 100).map((t: any) => ({ word: t.term, count: t.tf }));
  ...
  async exportAnalytics(companyId: string, body: ExportBody): Promise<Buffer> {
    ...
    const workbook = new ExcelJS.Workbook(); workbook.creator = 'Iuppy'; workbook.created = new Date();
    const overviewSheet = workbook.addWorksheet('Overview'); overviewSheet.addRow(['Métrica', 'Valor']); overviewSheet.addRow(['Total Formulários', overviewData.kpis.totalForms]); overviewSheet.addRow(['Total Submissões', overviewData.kpis.totalSubmissions]);
    const listSheet = workbook.addWorksheet('Lista de Forms'); listSheet.columns = [{ header: 'Formulário', key: 'title' }, { header: 'Status', key: 'status' }, { header: 'Envios', key: 'submissions' }, { header: '% No Prazo', key: 'onTimeRate' }]; listSheet.addRows((listData.items as any[]).map((r: any) => ({ ...r, title: getTranslation(r.title, 'pt-BR'), onTimeRate: (r.onTimeRate * 100).toFixed(1) })));
    if (body.formId && statsData && submissionsData && fieldsData) {
      const subSheet = workbook.addWorksheet('Submissões');
      const headers = fieldsData.map(f => ({ header: getTranslation(f.label, 'pt-BR'), key: f.fieldId }));
      subSheet.columns = [{ header: 'ID', key: 'submissionId' }, { header: 'Usuário', key: 'userName' }, { header: 'Data', key: 'submittedAt' }, ...headers];
      subSheet.addRows((submissionsData.items as any[]).map((s: any) => { const row: any = { ...s, userName: s.userName ?? s.userId }; (s.answers || []).forEach((a: any) => row[a.fieldId] = Array.isArray(a.value) ? a.value.join(', ') : a.value); return row; }));
    }
    return workbook.xlsx.writeBuffer() as unknown as Promise<Buffer>;
  }
  async jobs(companyId: string) { const lastReminders = await this.safeQuery(`SELECT MAX(ts) AS "lastRun" FROM reminder_event WHERE "companyId" = $1`, [companyId]); return { queues: [], uptime: 86400, lastRun: lastReminders?.[0]?.lastRun ?? null }; }
}
function getTranslation(field: TranslatableString | string | null | undefined, locale: string): string { if (!field) return ''; if (typeof field === 'string') return field; return field[locale] || field['pt-BR'] || field[Object.keys(field)[0]] || ''; }
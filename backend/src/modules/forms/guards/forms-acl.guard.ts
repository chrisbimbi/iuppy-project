...
export class FormsAclGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!userId || !companyId) {
      this.logger.warn(
        'ACL Guard: Usuário ou CompanyId não encontrado no token.',
      );
    ...
    if (!formId) {
      this.logger.warn(
        'ACL Guard: formId não encontrado nos parâmetros da rota.',
      );
    ...
    try {
      ...
      if (!form || form.length === 0) {
        throw new NotFoundException(
          'Formulário não encontrado ou não pertence a esta companhia.',
        );
      }
      ...
      this.logger.warn(
        `ACL Negado: Usuário ${userId} tentou aceder ao form ${formId} sem permissão.`,
      );
      throw new ForbiddenException(
        'Você não tem permissão para aceder a este formulário.',
      );
    } catch (e) {

...
export class CapabilitiesService {
    ...
    async forUser(companyId: string, user: { id: string; role: Role }): Promise<AccessCapabilities> {
        // SuperAdmin -> total
        if (user.role === Role.SuperAdmin) {
            const modules: AccessCapabilities['modules'] = {}
            ALL_MODULES.forEach(k => {
                modules[k] = { canView: true, canEdit: true, canManage: true, scopeType: 'ALL_SPACES' }
            })

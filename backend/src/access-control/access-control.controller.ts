...
export class AccessControlController {
    ...
    list(@Param('companyId') companyId: string, @Query('userId') userId?: string) {
        return this.service.list(companyId, userId)
    }

    @Post('grants')
    @Roles(Role.SuperAdmin, Role.CompanyAdmin)
    upsert(@Param('companyId') companyId: string, @Body() dto: UpsertAccessGrantDto) {
    ...
    me(@Param('companyId') companyId: string, @Req() req: any) {
        ...
        return this.service.capabilities(companyId, { id: String(user.id), role: user.role })
    }
    ...
    capabilitiesAdminView(
        ...
        return this.service.capabilities(companyId, { id: userId, role: Role.Viewer })
    }
}
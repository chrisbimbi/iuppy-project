...
import type { AccessGrant, AccessCapabilities, UpsertAccessGrantDto } from '@shared/types/Access'
import { Channel } from 'src/channels/channel.entity'
import { SpaceEntity } from 'src/spaces/space.entity'

const ALL_MODULE_KEYS: ModuleKey[] = [
  'news', 'channels', 'groups', 'surveys', 'forms', 'onboarding', 'training', 'jobs',
  'birthdays', 'recognition', 'quicklinks', 'benefits', 'vacations', 'podcasts', 'analytics', 'chat',
]
...
export class AccessControlService {
  constructor(
    @InjectRepository(ModuleAccessGrantEntity) private grants: Repository<ModuleAccessGrantEntity>,
  ...
  async upsert(companyId: string, dto: UpsertAccessGrantDto): Promise<AccessGrant> {
    ...
    if (norm.scopeType === 'SPACE_IDS') {
      if (!norm.spaceIds?.length) {
        throw new BadRequestException('spaceIds é obrigatório quando scopeType=SPACE_IDS')
      }
    }
  ...
  async capabilities(
    ...
    const inheritedSpaceIds = Array.from(new Set([...adminSpaceIds, ...extraSpaceIdsFromChannels]))

// src/app/modules/contents/components/Step2.tsx
...
export const Step2: React.FC<Step2Props> = ({
  ...
  const cancelGroups = () => {
    setSelectedGroups(data.settings.targetAudience || [])
    groupsModal?.hide()
  }

  const commitAuthor = () => {
    setFieldValue('authorId', selectedAuthor)
    authorModal?.hide()
  }
  const cancelAuthor = () => {
    setSelectedAuthor(null)
    setFieldValue('settings.showAuthor', false)
    authorModal?.hide()
  }

  const commitPush = () => pushModal?.hide()
  const cancelPush = () => {
    setFieldValue('settings.pushNotification', false)
    pushModal?.hide()
  }

  const commitShare = () => shareModal?.hide()
  const cancelShare = () => {
    setFieldValue('settings.allowSharing', false)
    shareModal?.hide()
  }
    ...
    <div className="w-100">
      ...
      <div className="pb-5">
        ...
        <div className="text-gray-400 fw-bold fs-6">
          Defina quem pode ver este conteúdo e envio de notificações
        </div>
      </div>
            ...
            {selectedGroups.length > 0 && (
              <div className="mt-2">
                {groups
                  ...
                  .map(g => (
                    <span key={g.id} className="badge badge-primary me-1">
                      {g.name}
                    </span>
                  ))}
              </div>
      ...
      <div className="pb-5">
        ...
        <div className="text-gray-400 fw-bold fs-6">
          Compartilhamento, comentários e reações
        </div>
      </div>
      <div className="row mb-10">
        ...
        <div className="col-md-4">
          <div className="form-check form-switch form-switch-custom form-switch-solid d-flex align-items-center">
            <input
              ...
              onChange={e => {
                handle('pushNotification', e.target.checked)
                if (e.target.checked) pushModal?.show()
              }}
            />
        ...
        <div className="col-md-4">
          <div className="form-check form-switch form-switch-custom form-switch-solid">
            <input
              className="form-check-input"
              type="checkbox"
              checked={data.settings.inAppNotification}
            ...
            <div className="col-md-4">
              <div className="form-check form-switch form-switch-custom form-switch-solid">
                <input
                  ...
                  onChange={e =>
                    handle('schedulePublication', e.target.checked)
                  }
                />
              ...
              {data.settings.schedulePublication && (
                <input
                  ...
                  value={
                    data.settings.schedulePublishDate
                      ? new Date(
                        data.settings.schedulePublishDate
                      )
                        .toISOString()
                  ...
                  onChange={e =>
                    handle('schedulePublishDate', e.target.value)
                  }
                />
              )}
            </div>
            <div className="col-md-4">
              <div className="form-check form-switch form-switch-custom form-switch-solid">
                <input
                  ...
                  onChange={e =>
                    handle('expirePublication', e.target.checked)
                  }
                />
                ...
                <input
                  ...
                  value={
                    data.settings.expirationDate
                      ? new Date(data.settings.expirationDate)
                        .toISOString()
                        .slice(0, 16)
                      : ''
                  }
                  onChange={e =>
                    handle('expirationDate', e.target.value)
                  }
                />
              )}
            </div>
      ...
      <div className="pb-5">
        ...
        <div className="text-gray-400 fw-bold fs-6">
          Autor, pinagem e confirmação de leitura
        </div>
      </div>
      <div className="row mb-10">
        <div className="col-md-4">
          ...
          {data.authorId && data.authorId !== currentUser!.id && data.settings.showAuthor && (
            <div className="card p-3 mt-2">
              <div className="d-flex align-items-center">
                <img
                  src={
                    users.find(u => u.id === data.authorId)?.avatarUrl ||
        ...
        <div className="col-md-4">
          <div className="form-check form-switch form-switch-custom form-switch-solid">
            <input
              className="form-check-input"
              type="checkbox"
              checked={data.settings.acknowledgementRequired}
            ...
            <div className="modal-body">
              <div className="mb-10">
                ...
                <input
                  ...
                  onChange={e => handle('pushTitle', e.target.value)}
                />
              </div>
              <div className="mb-10">
                ...
                <textarea
                  ...
                  onChange={e => handle('pushContent', e.target.value)}
                />
              </div>
            </div>
      ...
      <div className="modal fade" id="kt_modal_select_groups" tabIndex={-1}>
        <div className="modal-dialog modal-dialog-scrollable">
          <div className="modal-content">
            ...
            <div className="modal-body">
              {groups.map(g => (
                ...
                >
                  ...
                  />
                  <label className="form-check-label" htmlFor={`grp_${g.id}`}>
                    {g.name}
                  </label>
                </div>
              ))}
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-light" onClick={cancelGroups}>
                Cancelar
              </button>
              <button type="button" className="btn btn-primary" onClick={commitGroups}>
                OK
              </button>
            </div>
          </div>
      ...
      <div className="modal fade" id="kt_modal_share_options" tabIndex={-1}>
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-light" onClick={cancelShare}>
                Cancelar
              </button>
              <button type="button" className="btn btn-primary" onClick={commitShare}>
                OK
              </button>
            </div>
          </div>
      ...
      <div className="modal fade" id="kt_modal_select_author" tabIndex={-1}>
        <div className="modal-dialog modal-dialog-scrollable">
          <div className="modal-content">
                ...
                .map(u => (
                  ...
                  >
                    <input
                      ...
                      id={`auth_${u.id}`}
                      name="selectAuthor"
                      checked={selectedAuthor === u.id}
                      onChange={() => setSelectedAuthor(u.id)}
                    />
                    <label className="form-check-label" htmlFor={`auth_${u.id}`}>
                      {u.name}
                    </label>
                  </div>
                ))}
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-light" onClick={cancelAuthor}>
                Cancelar
              </button>
              <button type="button" className="btn btn-primary" onClick={commitAuthor}>
                OK
              </button>
            </div>
          </div>

...
import { api } from 'src/app/api' // <-- corrigido

export const ChannelsService = {
  async list(companyId: string, spaceId?: string): Promise<Channel[]> {
    const { data } = await api.get<Channel[]>('/channels', { params: { companyId, spaceId } })
    return data
  },
  async createChannel(channel: Partial<Channel>): Promise<Channel> {
    const { data } = await api.post<Channel>('/channels', channel); return data
  },
  async updateChannel(id: string, channel: Partial<Channel>): Promise<Channel> {
    const { data } = await api.put<Channel>(`/channels/${id}`, channel); return data
  },
